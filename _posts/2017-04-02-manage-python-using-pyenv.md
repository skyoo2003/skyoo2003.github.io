...
공식 Github 프로젝트에서 복제하여 가져온다. (최신 커밋을 git 명령을 통해 손쉽게 적용할 수 있음!) `$HOME/.pyenv` 에 설치하는 것을 가장 권장하고 있으나 필수는 아니므로 적절한 경로에 가져온다.
...
즉, `python 호출` -> `pyenv가 명령어 후킹` -> `우선 순위에 따라 사용할 버전 정보 획득` -> `해당 파이썬 버전 실행 (만약 선택한 버전이 3.3이라면, python3.3)` 과 같다.

추가로 특이한 점은 복수의 파이썬 버전을 선택할 수 있다는 점이다. 만약에 2.7.13과 3.4.6 버전을 사용하기로 한다면? `pyenv (local|global|shell) 2.7.13 3.4.6` 과 같이 설정하면 된다. 이렇게 설정한 경우, `python` 명령을 호출하면, 2.7.13 버전의 파이썬이 호출된다. 만약에 3.4.6 버전을 디폴트로 사용하고자 한다면? `pyenv (local|global|shell) 3.4.6 2.7.13` 과 같이 순서만 바꾸면 된다.

구체적인 내용은 아래에 정리해보도록 하겠다.

#### pyenv local

특정 디렉토리에서 파이썬 버전을 관리하기 위한 명령어이다. 더 정확히는 `$PYENV_DIR/.python-version`에 사용할 파이썬 버전을 정의하는 명령어라고 할 수 있다. 즉, pyenv가 python 커맨드를 후킹하는데, 이 때, 해당 경로의 파일에 버전이 정의되어 있으면, 해당 버전의 파이썬을 수행하도록 동작한다. 특별히 `$PYENV_DIR` 환경변수를 설정하지 않았다면, 현재 디렉토리에 해당 파일이 생성되는 것을 알 수 있다!
...
특정 시스템의 파이썬 버전을 관리하기 위한 명령이다. 더 정확히는 `$PYENV_ROOT/version`에 사용할 버전을 정의하기 위한 명령어이다. `$PYENV_ROOT` 환경변수를 특별히 정의하지 않았다면 기본적으로 `~/.pyenv` 를 기본값으로 가지고 있다.
